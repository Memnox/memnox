...
describe('FirewallSession — gating tools/call', () => {
  ...
  it('answers the client directly when the call is denied, and never forwards it', async () => {
  ...
  it('logs the block with the tool name and reason', async () => {
    ...
    expect(logs).toEqual(['denied tools/call "delete_repo": writes to production']);
  });

  it('denies a statically filtered tool without consulting the authorizer', async () => {
    const { session, channel, authorizer } = harness(
      ALLOW,
