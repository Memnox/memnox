...
export function runDoctor(input: DoctorInput): DoctorReport {
  ...
  for (const resource of input.resources) {
    ...
    findings.push({
      ...
      remediation: escalateProductionStep(id),
    });
  }
...
function escalateProductionStep(findingId: string): HardenStep {
  const file = `${POLICY_DIR}/escalate-production.yaml`;
  const contents = [
    'version: 1',
    'policies:',
    '  - name: escalate-production',
    '    match:',
    '      actions: ["*"]',
