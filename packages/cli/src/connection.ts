...
/** Flag, then environment, then what `memnox setup` stored on disk. */
...
/** The top-level handler cannot see a command's `--url`, so it printed the wrong one. */
...
/** "fetch failed" names neither the address nor the fix. */
