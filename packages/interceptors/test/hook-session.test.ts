...
describe('HookSession', () => {
  ...
  it('asks the person at the keyboard when the verdict asks', async () => {
    const { session } = sessionFor({
...
describe('the two-minute demo, offline', () => {
  it('denies a credential read and names the substitute, with no runtime at all', async () => {
    const session = new HookSession({
      authorizer: new HookAuthorizer({
        gate: new LocalGate([denyEnv], { agentName: 'claude-code' }),
        log: () => {},
      }),
      log: () => {},
    });
