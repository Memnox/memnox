...
import { isValidTimeWindow, matchesTimeWindow } from '../src/policy/time-window';
...
describe('time-scoped policies', () => {
  const outsideBusinessHours: Policy = {
    ...
    decision: { effect: DECISION_EFFECT.ASK, approvers: ['on-call'] },
  };
  ...
  it('requires approval for a weeknight deploy', () => {
    expect(evaluate(MONDAY_11PM).effect).toBe(DECISION_EFFECT.ASK);
  });

  it('requires approval all weekend', () => {
    expect(evaluate(SATURDAY_10AM).effect).toBe(DECISION_EFFECT.ASK);
  });
  ...
  it('applies the restriction when no instant is supplied', () => {
    ...
    expect(withoutTime.effect).toBe(DECISION_EFFECT.ASK);
  });
});
