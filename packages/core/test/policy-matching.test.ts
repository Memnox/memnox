...
const rule = (over: Partial<Policy> & Pick<Policy, 'name' | 'match'>): Policy => ({
  decision: { effect: DECISION_EFFECT.DENY, reason: over.name },
  ...over,
});
...
describe('argument matching', () => {
  ...
  it('narrows with each named argument — every one must match', () => {
    ...
    expect(inScope.effect).toBe(DECISION_EFFECT.DENY);
  ...
  it('applies a rule only on the branches it names', () => {
    const engineUnderTest = engine(
      rule({
        ...
        decision: { effect: DECISION_EFFECT.ASK, approvers: ['eng-lead'] },
      }),
    );
...
describe('per-rule monitor mode', () => {
  const observed = rule({
    ...
    decision: { effect: DECISION_EFFECT.DENY, mode: 'observe', reason: 'candidate' },
  });
  ...
  it('records what it would have done without applying it', () => {
    ...
    expect(result.effect).toBe(DECISION_EFFECT.ALLOW);
    expect(result.shadowEffect).toBe(DECISION_EFFECT.DENY);
  ...
  it('reports no denied effect when monitoring is no stricter than the verdict', () => {
    const result = engine(
      rule({
        name: 'observed-allow',
        match: { actions: ['deploy.*'] },
