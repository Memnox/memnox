  ...
  policies: [
    {
      ...
      decision: { effect: 'deny', reason: 'No AI database deletion' },
    },
  ],
};
...
describe('validatePolicyDocument', () => {
  ...
  it('rejects duplicate policy names and lists every issue at once', () => {
    const invalid = {
      version: 2,
      policies: [
        { name: 'dup', match: { actions: ['a'] }, decision: { effect: 'deny' } },
        { name: 'dup', match: { actions: ['b'] }, decision: { effect: 'deny' } },
      ],
    };
    try {
...
describe('validatePolicyDocument — argument, context and outcome fields', () => {
  const documentWith = (
    ...
    decision: Record<string, unknown> = { effect: 'deny' },
  ): unknown => ({
    version: 1,
  ...
  it('accepts the deny effect without demanding approvers', () => {
    const doc = validatePolicyDocument(documentWith({}, { effect: 'deny' }));

    expect(doc.policies[0]?.decision.effect).toBe('deny');
  });

  it('accepts observe mode and rejects any other mode', () => {
    const doc = validatePolicyDocument(
      documentWith({}, { effect: 'deny', mode: 'observe' }),
    );
    expect(doc.policies[0]?.decision.mode).toBe('observe');

    expect(() =>
      validatePolicyDocument(documentWith({}, { effect: 'deny', mode: 'maybe' })),
    ).toThrow(PolicyValidationError);
  });
