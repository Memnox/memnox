...
decision — **allow, withhold, or escalate to a person** — before anything runs.
A refusal names what to use instead, so the agent finishes the task under
constraint rather than abandoning it. The rules are YAML you commit. There is no
LLM in the decision path: same input, same decision, every time. It runs entirely
on your machine, with no account and no network call.
