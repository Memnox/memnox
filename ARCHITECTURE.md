...
| 4b. Census | `@memnox/organization`, `census-sources.ts`, `GET /v1/census`, `memnox census` | Four sources, each naming the record that proved an agent exists. A source that cannot be read is reported, so a small count is never mistaken for a clean one |
...
| 7d. State | `StateFact` (org-graph), `JsonFileStateStore`, `match.state` | Distributed inside the bundle rather than queried, so a freeze refuses in microseconds. Every fact carries a mandatory `validUntil` |
...
| 10c. Readiness | `ReadinessService`, `GET /v1/agents/:id/readiness`, `memnox readiness` | Every item is a query against a store that exists. An item nothing answers is unknown, which is not a pass |
