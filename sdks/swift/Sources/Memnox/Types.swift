...
public struct Decision: Decodable, Sendable {
    ...
    public let withheldEffect: Effect?

    public var allowed: Bool { effect == .allow }

    /// True when monitor mode let this through but policy would have stopped it.
    public var wouldHaveStopped: Bool { withheldEffect != nil }

    private enum CodingKeys: String, CodingKey {
        case eventId, effect, reason, matchedPolicies, approvalId, withheldEffect
    }
    ...
    public init(from decoder: Decoder) throws {
        ...
        withheldEffect = try container.decodeIfPresent(Effect.self, forKey: .withheldEffect)
    }
}
...
public enum MemnoxError: Error, Sendable {
    ...
    case blocked(reason: String, eventId: String)
...
extension MemnoxError: CustomStringConvertible {
    public var description: String {
        switch self {
        case let .blocked(reason, _): return "blocked by policy: \(reason)"
